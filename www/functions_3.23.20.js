...
String.prototype.replaceAll = function (find, replace) {
   ...
   //   height: height,
      ...
      symmetric: false,
      array: LCL_bar3.map(function(x) {return x;}),
      arrayminus: UCL_bar3.map(function(x) {return x;}),
      color: 'black',
      ...
      var lcl_char = ci_test[0];
      var ucl_char = ci_test[ci_test.length - 1];
      var lcl_num = ses.slice(0,1).toString().split(',').map(Number);
      var ucl_num = ses.slice(-1).toString().split(',').map(Number);
      //var hovertemplate ="hovertemplate";
      if(y.length == 0) {return null; }
